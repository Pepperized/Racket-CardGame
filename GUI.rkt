...
(define creatureObjects '())
                                                                                                                                                ;creatureObjects selected by P1          ;creatureObjects selected by P2
(define selectedCreatureObjects (list (list config:noCreature config:noCreature) (list config:noCreature config:noCreature))) ; format: '( '(P1CreatureObjectIndex P2CreatureIndex) '(P1CreatureObjectIndex P2CreatureIndex) )
(define selectedCreatures (list (list config:noCreature config:noCreature) (list config:noCreature config:noCreature)))
...
(define removeObject (λ (index)
                       (set! sortedObjects (remove (list-ref sortedObjects index) sortedObjects))    ;Joe altered for tomorrow
                       (set! objects sortedObjects)
                       (send canvas on-paint)))

(define removeCreatureObject (λ (index)
                       (set! creatureObjects (remove (list-ref creatureObjects index) creatureObjects))    ;Joe altered for tomorrow
                       (send canvas on-paint)))
                         ...
                         ((is-a? defender creature%)
                          (cond
                            ((>= (send attacker get-attack) (send defender get-life))
                             ...
                             (refreshBoard))
                            ((< (send attacker get-attack) (send defender get-life)) #f)))
                         (#t #f)))))
